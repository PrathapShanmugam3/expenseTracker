...
class StatementParserService {
  ...
  static Future<bool> isPdfPasswordProtected(PlatformFile file) async {
    try {
      final bytes = kIsWeb ? file.bytes! : await File(file.path!).readAsBytes();
      
      // Attempt to load the document
      // If it is password protected, this will throw an exception
      final document = PdfDocument(inputBytes: bytes);
      document.dispose();
      
      return false; // Opened successfully, so no password needed
    } catch (e) {
      // If error occurs, assume it is password protected
      debugPrint("[StatementParser] PDF Check Error (Likely Encrypted): $e");
      return true;
    }
  }

  /// Upload statement to FastAPI backend
  static Future<Map<String, dynamic>> uploadStatement(PlatformFile file, int userId, {String? password}) async {
    debugPrint("[StatementParser] Uploading file: ${file.name} for user: $userId");
    
    // Use the provided FastAPI URL
    final String url = 'https://fastapi-plch.onrender.com/api/v1/upload-statement';
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        debugPrint("[StatementParser] Sending request to $url (Attempt ${retryCount + 1})...");
        
        // Send request
        final streamedResponse = await request.send();
        final response = await http.Response.fromStream(streamedResponse);

        debugPrint("[StatementParser] Response Status: ${response.statusCode}");
        debugPrint("[StatementParser] Response Data: ${response.body}");
        ...
        debugPrint("[StatementParser] Error uploading file: $e");
        if (retryCount < maxRetries) {
          retryCount++;
          await Future.delayed(Duration(seconds: 2 * retryCount));
          continue;
        }
        throw Exception('Error uploading file after $maxRetries retries: $e');
      }
    }
    throw Exception('Upload failed');
  }

  /// Pick a file and parse it locally (no backend upload)
  static Future<List<Map<String, dynamic>>> pickAndParseStatement() async {
    debugPrint("[StatementParser] Started");
    try {
      ...
      if (result != null) {
        ...
        debugPrint("[StatementParser] File picked: ${file.name}");

        return await _parseFile(file);
      }
    } catch (e, stackTrace) {
      debugPrint("[StatementParser] Error picking/parsing file: $e");
      debugPrint("[StatementParser] Stack trace: $stackTrace");
    }
    return [];
  }

  static Future<List<Map<String, dynamic>>> _parseFile(PlatformFile file) async {
    try {
      final fileName = file.name.toLowerCase();
      List<Expense> expenses = [];

      if (fileName.endsWith('.pdf')) {
        expenses = await _parsePdf(file);
      } else if (fileName.endsWith('.csv')) {
        expenses = await _parseCsv(file);
      }

      debugPrint("[StatementParser] Parsed ${expenses.length} expenses");
      ...
      debugPrint("[StatementParser] Parse error: $e");
      return [];
    }
  }

  /// Parse PDF file
  static Future<List<Expense>> _parsePdf(PlatformFile file) async {
    try {
      final bytes = kIsWeb ? file.bytes! : await File(file.path!).readAsBytes();
      final document = PdfDocument(inputBytes: bytes);
      ...
      debugPrint('[StatementParser] PDF Parse Error: $e');
      throw Exception('Failed to parse PDF: $e');
    }
  }

  /// Parse CSV file
  static Future<List<Expense>> _parseCsv(PlatformFile file) async {
    try {
      ...
      debugPrint('[StatementParser] CSV Parse Error: $e');
  ...
  static List<Expense> _parseCsvRows(List<List<dynamic>> rows) {
    ...
    debugPrint('[StatementParser] Detected CSV Format: $format');
